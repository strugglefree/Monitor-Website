...
public class TerminalWebSocket {
    ...
    public void onOpen(Session session,
        ...
        if(client == null || ssh == null) {
            session.close(new CloseReason(CloseReason.CloseCodes.CANNOT_ACCEPT, "无法识别此主机"));
            return;
        }
        if (this.createSshConnection(session, ssh, client.getIp())){
            log.info("主机 {} 的SSH连接已创建", client.getIp());
        }
    }
    ...
    public void onClose(Session session) throws IOException {
        ...
        if (shell != null) {
            ...
            log.info("主机 {} 的SSH连接以断开", shell.js.getHost());
        }
    }
